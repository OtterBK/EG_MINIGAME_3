...
public class Hunter extends Ability{
	...
	public Hunter(HRWBase hrw, Player p, String abilityName, Team team) {
		...
		weapon = new ItemStack(Material.GOLD_AXE, 1);
		...
		cooldown_teritarySkill = 8;
		cooldown_ultimateSkill = 39;
	...
	public void respawned() {
		cooldown.setCooldownSpec("primary", 3);
	}
	...
	public void invenHelper(Player p) {
		p.openInventory(hrw.inven_desc_hunter);
	}
	...
	public void updateStatItem() {
		...
		statLoreList.add("");
		statLoreList.add("§7적을 띄워올린 횟수 : §a"+hrwData.airboneCnt+"  ");
		statLoreList.add("");
	...
	public void primarySkill(){
		...
		if(cooldown.checkCooldown("primary") && !p.isDead()){
			...
			p.getWorld().playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 2.0f, 3.0f);
			p.getWorld().spawnParticle(Particle.CLOUD, p.getLocation(), 2, 0.5F, 0.5f, 0.5f, 0.1f);
			
			customSpeedUp(p, p, 0.65f, 5, false);
		} else {
			p.playSound(p.getLocation(), Sound.UI_BUTTON_CLICK, 1.2f, 1.8f);
		}
	}
	...
	public void secondarySkill(){
		...
		if(cooldown.checkCooldown("secondary") && !p.isDead()){
			...
			ef.particle = Particle.SWEEP_ATTACK;
			ef.iterations = 1;
			ef.setLocation(l.add(0,-0.7f,0));
			ef.setTargetLocation(t.add(0,0.7f,0));
			ef.start();
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(hrw.server, new Runnable() {
				public void run() {
					...
					ef.particle = Particle.SWEEP_ATTACK;
					ef.iterations = 1;
					ef.setLocation(l1.add(0,0.7f,0));
					ef.setTargetLocation(t1.add(0,-0.7f,0));
					ef.start();
				}
	...
	public void teritarySkill(){
		...
		if(cooldown.checkCooldown("teritary") && !p.isDead()){
			...
			if(hit != null) {
				Player t = hit.get(0);
				customDamage(t, p, 13, false);
				customAirbon(t, p, 1.5f, false);
			} 				
			LineEffect ef = new LineEffect(effectManager);
			Location l = p.getLocation().add(p.getLocation().getDirection().multiply(0.75f));
			ef.particle = Particle.REDSTONE;
			ef.color = Color.fromRGB(255, 255, 0);
			ef.iterations = 1;
			ef.setLocation(l);
			ef.setTargetLocation(l.add(0,2,0));
			ef.start();
	...
	public void ultimateSkill(){
		...
		if(cooldown.checkCooldown("ultimate")){
			...
			blockPerUlti = true;
			p.playSound(p.getLocation(), Sound.ENTITY_ARROW_SHOOT, 1.2f, 0.2f);
			...
			vp.setOnHit(new Runnable() {
				public void run() {
					...
					Location tl = t.getLocation().subtract(t.getEyeLocation().getDirection());
					tl.setY(t.getLocation().getY());
					p.teleport(tl, TeleportCause.PLUGIN);
					customDamage(t, p, 25, false);
					t.getWorld().playSound(t.getLocation(), Sound.ENTITY_PLAYER_HURT, 1.5f, 1.2f);
					p.getWorld().playSound(p.getLocation(), Sound.ENTITY_MAGMACUBE_JUMP, 2.0f, 0.5f);
				    CircleEffect particle = new CircleEffect(effectManager);
				    particle.setEntity(p);
				    particle.period = 0;
				    particle.iterations = 1;
				    particle.particle = Particle.SLIME;
				    particle.radius = 1f;
				    particle.start();
					blockPerUlti = false;
				}
